...
// 检查特殊用户ID是否冲突
if (isset($domain) && isset($DOMAIN) && $domain == $DOMAIN && isset($OJ_NAME) && $OJ_NAME == $user_id) {
    print "<script language='javascript'>\n";
    print "alert('$MSG_USER_ID Existed!\\n');\n";
    print "history.go(-1);\n</script>";
    exit(0);
...
if (isset($_SERVER['HTTP_X_FORWARDED_FOR']) && !empty(trim($_SERVER['HTTP_X_FORWARDED_FOR']))) {
    ...
    $ip = (htmlentities(trim($tmp_ip[0]), ENT_QUOTES, "UTF-8"));
...
if (isset($OJ_REG_SPEED) && $OJ_REG_SPEED > 0) {
    ...
    if ($count > $OJ_REG_SPEED) {
        ...
        if (isset($OJ_ADMIN) && $OJ_ADMIN != "root@localhost") {
            email($OJ_ADMIN, "系统警告,疑似攻击!", $warning . "\n from " . (isset($domain) ? $domain : 'unknown'));
        }
        print "<script language='javascript'>\n";
...
// 获取用户组映射函数
$group_name = "";
if (function_exists('getMappedSpecial')) {
    $group_name = getMappedSpecial($user_id);
}
...
$sql = "INSERT INTO `users`("
        ...
        . "VALUES(?,?,?,NOW(),?,NOW(),?,?,?,?,?)";
$rows = pdo_query($sql, $user_id, $email, $ip, $password, $nick, $school, $group_name, $defunct, $_SESSION[$OJ_NAME . '_' . 'activecode']);// or die("Insert Error!\n");
...
if (isset($OJ_EMAIL_CONFIRM) && $OJ_EMAIL_CONFIRM) {
    ...
    if (isset($OJ_TEMPLATE) && file_exists("template/" . $OJ_TEMPLATE . "/error.php")) {
...
if (!isset($OJ_REG_NEED_CONFIRM) || !$OJ_REG_NEED_CONFIRM) {
    ...
    if ($rows_cnt == 1) {
        ...
        if (isset($OJ_SaaS_ENABLE) && $OJ_SaaS_ENABLE && isset($domain) && isset($DOMAIN) && $domain == $DOMAIN) {
